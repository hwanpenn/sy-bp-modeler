...
angular.module('flowableModeler')
  .controller('ProcessesCtrl', ['$rootScope', '$scope', '$translate', '$http', '$timeout','$location', '$modal', function ($rootScope, $scope, $translate, $http, $timeout, $location, $modal) {
	  ...
	  $scope.Filtercategory = function(categoryvalue) {
		  alert(categoryvalue);
	  };
	  ...
	  $scope.loadProcessCategory = function() {
		  $scope.model.loading = true;
		  ...
		  $http({method: 'GET', url: FLOWABLE.CONFIG.contextRoot + '/app/rest/categorys', params: params}).
		  	success(function(data, status, headers, config) {
	    		$scope.categorys = data.items;
	    		$scope.model.loading = false;
	        }).
	        error(function(data, status, headers, config) {
	           console.log('Something went wrong: ' + data);
